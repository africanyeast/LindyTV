...
let channelNumber = 1;
let isMin = false, isMuted = false, isOn = true, showInfo = false;
let user = "akin";
let baseUrl = `../data/${user}`
...
function getChannelName(channel) {
    ...
    // Fetch channels.json
    xhttp.open("GET", `${baseUrl}/channels.json?t=${Date.now()}`, false);
    xhttp.send();
    
    if (xhttp.status === 200) {
        let channelsData = JSON.parse(xhttp.responseText);
        
        // Find the channel name by its number
        for (const [channelName, channelInfo] of Object.entries(channelsData)) {
            if (channelInfo.channel_no === channel) {
                name = channelName;
                break;  // Stop once the channel name is found
            }
        }
    }
...
function getList() {
    ...
    // Create the dynamic filename with the folder path
    const playlistFile = `${baseUrl}/playlists/playlist-${formattedDate}.json`;
    ...
    // Send the request to the dynamically generated playlist filename
    xhttp.open("GET", playlistFile + "?t=" + Date.now());
    xhttp.send();
}
...
function toggleMute() {
    if (player.isMuted()) {
        player.unMute();
        isMuted = false;
        muteIcon.src = "src/icons/volume-2.svg";
    } else {
        muteIcon.src = "src/icons/volume-x.svg";
        player.mute();
        isMuted = true;
    }
}
...
function switchChannel(a) {
    if (isOn) {
        // remove the currently playing video title on the screen
        document.getElementById("title").innerText = "";
        // stop the currently playing video on the screen
        player.stopVideo();
        channelNumber += a;
        if (channelNumber < 1) {
...
function toggleControl() {
    ...
    if (isMin) {
        ...
        minimizeImg.src = "src/icons/minimize-2.svg";
        isMin = false;
    } else {
        ...
        minimizeImg.src = "src/icons/maximize.svg";
        isMin = true;
    }
}
